// seleting elements by get elements by id.
...
async function loadMovies(searchTerm) {
  await fetch(`https://omdbapi.com/?s=${searchTerm}&page=1&apikey=d8260958`)
...
function displayMovieList(movies) {
  ...
  for (let idx = 0; idx < movies.length; idx++) {
    ...
    // appending in the original data.
...
function displayMovieDetails(details) {
  resultGrid.innerHTML = `
    <div id = "movie-image">
        ...
        }" alt = "movie poster" class="rounded shadow-2xl">
    </div>
    <div id = "movie-info">
        <h1 class = "movie-title">${details.Title}</h1>
	  <p>Year: ${
      details.Year
    } &nbsp;<span class=" bg-yellow-500 p-1 rounded">Rating: ${
    details.Rated
  }</span> &nbsp;Released: ${details.Released}</p>
	  <p class="rounded shadow-md bg-zinc-700 p-1.5 w-fit"><span>Genere:</span> ${
      details.Genre
    }</p>
	  ...
	  <p><span><i class="fa-solid fa-award"></i></span>&nbsp;&nbsp;  ${
      details.Awards
    }</p>
    </div>
    `
